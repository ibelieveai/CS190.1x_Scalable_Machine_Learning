...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda a: LabeledPoint((a.label - 1922),a.features))
#parsedData.map
...
labelsAndPredsTrain = parsedTrainData.map(lambda x:(x.label,53.9316))
#labelsAndPredsTrain.take(10)                                          
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda x:(x.label,53.9316))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda x:(x.label,53.9316))
